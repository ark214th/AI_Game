(() => {
  ...
  const ui = {
    hud:$('hud'), joy:$('joy'), stick:$('stick'), start:$('start'), level:$('level'), paused:$('paused'), end:$('end'), cards:$('cards'),
    hpt:$('hpt'), hp:$('hp'), xpt:$('xpt'), xp:$('xp'), lv:$('lv'), tm:$('tm'), ko:$('ko'), weapons:$('weapons'), notice:$('notice'),
    boss:$('boss'), bossbar:$('bossbar'), startBtn:$('startBtn'), pauseBtn:$('pause'), resumeBtn:$('resume'), finishBtn:$('finishBtn'), retryBtn:$('retry'), soundBtn:$('sound'),
    endIcon:$('endico'), endTitle:$('endtitle'), endText:$('endtext'), resTime:$('restime'), resKo:$('resko'), resLv:$('reslv')
  };
  ...
  let last=0, elapsed=0, spawnClock=0, bossClock=0, bonus=false, boss=null;
  ...
  const weapons = {
    moon:{icon:'🌙',name:'月光弾',level:1,max:7},
    orbit:{icon:'🪐',name:'守護星',level:0,max:6},
    nova:{icon:'❄️',name:'星霜の波',level:0,max:6},
    aura:{icon:'🧄',name:'聖なる香気',level:0,max:7},
    light:{icon:'⚡',name:'天雷',level:0,max:6},
    blade:{icon:'🌒',name:'三日月刃',level:0,max:6}
  };

  const weaponUpgrades = Object.entries(weapons).map(([id,w]) => ({
    id, icon:w.icon, name:w.name, max:w.max,
    get level(){ return weapons[id].level; },
    desc: {
      moon:'近い敵へ弾を放つ。弾数・威力・貫通力が増える。',
      orbit:'周囲を回る星が、触れた敵を傷つける。',
      nova:'一定時間ごとに広範囲へ冷気の波を放つ。',
      aura:'聖なる香りで、近くの敵へ継続ダメージ。',
      light:'複数の敵へ連鎖する雷を落とす。',
      blade:'貫通する三日月形の刃を周囲へ投げる。'
    }[id],
    apply(){ weapons[id].level++; }
  }));
  ...
  function reset(){
    elapsed=0;spawnClock=0;bossClock=0;bonus=false;boss=null;kills=0;shake=0;noticeTimer=0;ended=false;paused=false;choosing=false;
    ...
    Object.values(weapons).forEach(w=>w.level=0);weapons.moon.level=1;
  ...
  function startGame(){
    audioOn();reset();ui.start.classList.add('hidden');ui.end.classList.add('hidden');ui.paused.classList.add('hidden');ui.hud.classList.remove('hidden');ui.joy.classList.remove('hidden');
    running=true;last=performance.now();showNotice('夜が始まった…',2);requestAnimationFrame(loop);
  }
  ...
  function spawnEnemy(type='slime', rank='normal'){
    ...
    const e={id:nextEnemyId++,x:player.x+Math.cos(a)*d,y:player.y+Math.sin(a)*d,r:16,hp:31,maxHp:31,speed:47,damage:10,xp:1,type,rank,dead:false,flash:0,touch:0,orbit:0,aura:0};
    if(type==='bat')Object.assign(e,{r:13,hp:20,maxHp:20,speed:92,damage:9,xp:1});
    else if(type==='skel')Object.assign(e,{r:17,hp:43,maxHp:43,speed:55,damage:13,xp:2});
    else if(type==='brute')Object.assign(e,{r:27,hp:125,maxHp:125,speed:34,damage:22,xp:5});
    const phase=Math.max(0,elapsed-45);
    const baseScale=1+phase/700;
  ...
  function spawnBoss(isMini=false){
    ...
    const a=Math.random()*TAU,d=Math.max(W,H)*.62+170;
    const bonusScale=1+Math.max(0,elapsed-GOAL_TIME)/120;
    boss={id:nextEnemyId++,type:'boss',rank:isMini?'mini':'boss',x:player.x+Math.cos(a)*d,y:player.y+Math.sin(a)*d,r:isMini?40:52,hp:(isMini?1500:2800)*bonusScale,maxHp:0,speed:isMini?38:29,damage:isMini?24:30,xp:isMini?28:45,dead:false,flash:0,touch:0,orbit:0,aura:0};
    boss.maxHp=boss.hp;enemies.push(boss);ui.boss.style.display='block';showNotice(isMini?'⚠ 月影の番人が現れた！':'⚠ 宵闇の王が現れた！',2.5);beep(110,.45,'sawtooth',.06);
  }

  function updateSpawns(dt){
    if(elapsed>=FIRST_BOSS_TIME&&!boss&&bossClock===0)spawnBoss(false);
    ...
    const missing=Math.max(0,target-enemies.filter(e=>!e.dead&&e.type!=='boss').length);
    const count=Math.min(bonus?5:4,missing);
    const bonusSecs=Math.max(0,elapsed-GOAL_TIME);
    const eliteChance=bonus?Math.min(.30,.12+bonusSecs/600):Math.min(.12,elapsed/1800);
    const giantChance=bonus?Math.min(.08,.025+bonusSecs/1800):.01;
    for(let i=0;i<count;i++){
      ...
      const rr=Math.random();const rank=rr<giantChance?'giant':rr<giantChance+eliteChance?'elite':'normal';spawnEnemy(type,rank);
    }
  ...
  function fireMoon(){const l=weapons.moon.level,n=1+Math.floor(l/2),targets=nearest(n);if(!targets.length)return;for(let i=0;i<n;i++){const t=targets[i%targets.length],a=Math.atan2(t.y-player.y,t.x-player.x);pushLimited(shots,{kind:'moon',x:player.x,y:player.y,r:6,vx:Math.cos(a)*500,vy:Math.sin(a)*500,life:1.5,damage:(10+l*6)*attackPower(),pierce:l>=7?2:l>=5?1:0,hit:new Set()},LIMITS.shots);}beep(690);}
  function fireBlades(){const l=weapons.blade.level,n=2+l,o=elapsed*.8;for(let i=0;i<n;i++){const a=o+i*TAU/n;pushLimited(shots,{kind:'blade',x:player.x,y:player.y,r:11,vx:Math.cos(a)*(275+l*16),vy:Math.sin(a)*(275+l*16),life:1.25+l*.08,damage:(16+l*9)*attackPower(),pierce:3+Math.floor(l/2),hit:new Set(),rot:a},LIMITS.shots);}}
  ...
  function dropGem(x,y,value,big=false){
    if(gems.length>=LIMITS.gems){const g=gems[Math.floor(Math.random()*gems.length)];if(g){g.value+=value;g.r=Math.min(10,g.r+.2);}return;}
    gems.push({x,y,r:big?7:5,value,vx:rand(-30,30),vy:rand(-30,30),life:0,dead:false,collect:false});
  }
  function killEnemy(e){
    e.dead=true;kills++;shake=e.type==='boss'?12:2;
    const count=e.type==='boss'?16:Math.max(1,Math.ceil(e.xp/3));for(let i=0;i<count;i++)dropGem(e.x+rand(-12,12),e.y+rand(-12,12),e.xp/count,e.type==='boss');
    if(e.type==='boss'){boss=null;ui.boss.style.display='none';showNotice(bonus?'中ボス撃破！ まだまだ続く！':'宵闇の王を倒した！',2);beep(880,.5,'triangle',.07);if(!bonus&&elapsed>=GOAL_TIME)enterBonus();}
  }
  ...
  function buildUpgradePicks(){
    const picks=[];
    const owned=shuffle(ownedWeapons());
    if(owned.length)picks.push(owned[0]);
    const pool=[];
    pool.push(...owned.filter(u=>!picks.includes(u)));
    pool.push(...newWeapons());
    pool.push(...passives.filter(p=>p.level<p.max));
    for(const u of shuffle(pool)){if(picks.length>=3)break;if(!picks.includes(u))picks.push(u);}
    const rewards=shuffle(repeatRewards.filter(r=>!('maxUses' in r)||r.uses<r.maxUses));
    for(const r of rewards){if(picks.length>=3)break;picks.push(r);}
  ...
  function openUpgrade(){
    ...
    for(const u of buildUpgradePicks()){
      ...
      const detail=u.repeat?'何度でも取得可能':`現在 Lv.${u.level} → Lv.${u.level+1}`;
  ...
  function update(dt){
    ...
    if((player.moonTimer-=dt)<=0){fireMoon();player.moonTimer=Math.max(.2,.78-weapons.moon.level*.075)*attackHaste();}
  ...
  function updateHud(){
    ...
    ui.tm.textContent=bonus?'BONUS '+formatTime(elapsed-GOAL_TIME,true):formatTime(Math.max(0,GOAL_TIME-elapsed),false);
    if(boss&&!boss.dead)ui.bossbar.style.width=clamp(boss.hp/boss.maxHp*100,0,100)+'%';
  ...
  function draw(){
    ...
    for(const e of enemies){if(e.dead)continue;const s=world(e.x,e.y);if(s.x<-90||s.x>W+90||s.y<-90||s.y>H+90)continue;ctx.save();ctx.translate(s.x,s.y);if(e.rank==='elite'){ctx.strokeStyle='#ffd75e';ctx.lineWidth=5;}else if(e.rank==='giant'){ctx.strokeStyle='#ff7a4f';ctx.lineWidth=7;}else{ctx.strokeStyle='#20213c';ctx.lineWidth=3;}ctx.fillStyle=e.flash>0?'#fff':e.type==='boss'?'#b738d3':e.type==='bat'?'#a978ff':e.type==='skel'?'#d8dddb':e.type==='brute'?'#d96374':'#69d69a';ctx.beginPath();ctx.arc(0,0,e.r,0,TAU);ctx.fill();ctx.stroke();ctx.fillStyle='#182034';ctx.font=`${Math.max(14,e.r)}px sans-serif`;ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(e.type==='boss'?'👾':e.type==='bat'?'🦇':e.type==='skel'?'💀':e.type==='brute'?'👹':'•',0,1);if(e.rank==='elite'){ctx.fillStyle='#ffd75e';ctx.font='12px sans-serif';ctx.fillText('★',0,-e.r-8);}if(e.rank==='giant'){ctx.fillStyle='#ffad72';ctx.font='15px sans-serif';ctx.fillText('◆',0,-e.r-10);}ctx.restore();}
    for(const s of shots){const p=world(s.x,s.y);ctx.fillStyle=s.kind==='blade'?'#a9f0ff':'#c7b7ff';ctx.beginPath();ctx.arc(p.x,p.y,s.r,0,TAU);ctx.fill();}
    for(let i=0;i<(weapons.orbit.level?1+weapons.orbit.level:0);i++){const a=player.orbitAngle+i*TAU/(1+weapons.orbit.level),p=world(player.x+Math.cos(a)*(62+weapons.orbit.level*3),player.y+Math.sin(a)*(62+weapons.orbit.level*3));ctx.fillStyle='#ffec8d';ctx.beginPath();ctx.arc(p.x,p.y,10,0,TAU);ctx.fill();}
    ...
    const p=world(player.x,player.y);ctx.fillStyle='#e9e8ff';ctx.strokeStyle='#5f54be';ctx.lineWidth=4;ctx.beginPath();ctx.arc(p.x,p.y,player.r,0,TAU);ctx.fill();ctx.stroke();if(player.shield){ctx.strokeStyle='#a9ecff';ctx.lineWidth=3;ctx.beginPath();ctx.arc(p.x,p.y,player.r+7,0,TAU);ctx.stroke();}
  ...
  ui.startBtn.onclick=startGame;ui.retryBtn.onclick=startGame;ui.pauseBtn.onclick=()=>{if(!running||choosing)return;paused=true;ui.paused.classList.remove('hidden');};ui.resumeBtn.onclick=()=>{ui.paused.classList.add('hidden');paused=false;last=performance.now();};ui.finishBtn.onclick=()=>finish(true);ui.soundBtn.onclick=()=>{soundOn=!soundOn;ui.soundBtn.textContent=soundOn?'🔊 効果音 ON':'🔇 効果音 OFF';if(soundOn)audioOn();};
  addEventListener('resize',resize,{passive:true});resize();reset();draw();
})();
