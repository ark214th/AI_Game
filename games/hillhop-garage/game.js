...
function refreshMenu(){
 ...
 $('routeHint').textContent=selected===3?'前回と同じ道に、何度でも挑戦できます。':'失敗しても、集めたコインは持ち帰れます。';
 ...
 $('heroNote').textContent=tab==='garage'?'強化した分だけ、遠くへ行ける。':save.runs===0?'下りで勢いをつけて、次の丘へ。':save.records[selected].distance?`前回までの最高記録 ${meters(save.records[selected].distance)}`:'新しい景色が、あなたを待っています。';
 ...
 $('runCount').textContent=`${save.runs} RUNS`;$('recordList').innerHTML=REGIONS.map((r,i)=>{const rec=save.records[i];return`<div class="record-card"><div class="record-top"><b>${r.name}</b><small>${rec.distance?meters(rec.distance):'—'}${rec.time?` · ${timeString(rec.time)}`:''}</small></div><div class="medals">${['完走','コイン60%回収','燃料20%を残す'].map((m,j)=>`<span class="medal ${rec.medals&(1<<j)?'earned':''}">${rec.medals&(1<<j)?'★':'☆'} ${m}</span>`).join('')}</div></div>`;}).join('')+(save.unlocked===3?`<div class="record-card"><div class="record-top"><b>果てしない遠征</b><small>${meters(save.records[3].distance)}</small></div></div>`:'');
...
function startRun(sameSeed=true){
 ...
 callout('LET’S ROLL!',1.5);if(save.runs<2)tip('アクセルで走ろう。下りでは離しても進めます。',5);updateHUD();
}
...
function showResult(){
 ...
 let message=clear?resultData.newRegion?(selected===2?'3つの地域を踏破！ 果てしない遠征が開きました。':`${REGIONS[selected+1].name}への道が開きました。`):'いい走りでした。次は、もう一つのメダルへ。':run.result.reason==='fuel'?'燃料切れ。下りでアクセルを離すと、もっと遠くへ。':'落とし穴に落ちてしまいました。手前から勢いをつけて跳ぼう。';
...
function render(){
 ...
 if(menu){
  ...
  const nextGap=tr.gaps.find(g=>g.a>run.x&&g.a-run.x<1100);if(nextGap&&nextGap.a-run.x>150){const p=screenPos(nextGap.a,tr.height(nextGap.a));rounded(clamp(p.x-50,130,w-150),148,116,31,15,'#fff5d4e8');text(`↟ 穴まで ${Math.ceil((nextGap.a-run.x)/10)} m`,clamp(p.x+8,188,w-92),168,11,'#966638');}
...
function updateHUD(){if(!run)return;$('distanceLabel').textContent=meters(run.distance);$('routeFill').style.width=`${run.track.endless?Math.min(100,(run.distance%1000)/10):clamp(run.x/run.track.length*100,0,100)}%`;$('fuelValue').textContent=Math.ceil(run.fuel);$('fuelFill').style.width=`${Math.max(0,run.fuel/run.maxFuel*100)}%`;$('fuelFill').parentElement.parentElement.classList.toggle('low',run.fuel<20);$('ecoLabel').textContent=run.fuel<=0?'EMPTY':!input.gas&&run.vx>40?'ECO':run.turbo>0?'BOOST':'FUEL';$('runCoins').textContent=format(run.coins+Math.floor(run.distance/100)*Math.round(2*activeRegion.reward));const fuel=run.track.items.find(i=>i.type==='fuel'&&!i.taken&&i.x>run.x);$('fuelNext').textContent=run.fuel<40?(fuel?`次の燃料まで ${Math.ceil((fuel.x-run.x)/10)} m`:run.track.endless?'この先に補給あり':`ゴールまで ${Math.max(0,Math.ceil((run.track.length-run.x)/10))} m`):'下りはアクセルを離して節約';$('effectBadges').innerHTML=(run.magnet>0?`<span>∩ MAGNET ${Math.ceil(run.magnet)}s</span>`:'')+(run.turbo>0?`<span>ϟ TURBO ${Math.ceil(run.turbo)}s</span>`:'');}
function updateTutorial(){if(!run)return;const show=(id,cond,msg,dur=4)=>{if(cond&&!tutorialSeen.has(id)){tutorialSeen.add(id);tip(msg,dur);}};
 if(save.runs<3){show('jump',run.x>1700,'ジャンプを短く押すと、小さく跳べます。');show('coast',run.x>6200,'下りでアクセルを離すと、燃料を節約できます。');show('height',run.x>11400,'高いコインは長押しジャンプ。地面の燃料も忘れずに。');}
 show('stall',run.time>8&&run.grounded&&Math.abs(run.vx)<8&&run.fuel>20,'登れないときは、少し下がって助走。エンジン強化も力になります。',5);
 show('fuel',run.fuel<20&&run.fuel>0,'燃料が少なくなりました。惰性を使って、次の補給へ。',3);
...
function frame(ts){const dt=last?Math.min((ts-last)/1000,.05):1/60;last=ts;clock+=dt;
 if(mode==='playing'&&run){accumulator+=dt;while(accumulator>=1/120&&mode==='playing'){run.update(1/120,input);processEvents();accumulator-=1/120;}wheelSpin+=run.vx*dt/22;const targetZoom=lerp(1,.78,clamp((Math.abs(run.vx)-200)/215,0,1));zoom=lerp(zoom,targetZoom,1-Math.exp(-dt*2));camX=lerp(camX,run.x+Math.max(0,run.vx-180)*.23,1-Math.exp(-dt*5));camY=lerp(camY,run.y+Math.max(-50,Math.min(55,run.vy*.1)),1-Math.exp(-dt*3));bankTimer+=dt;if(bankTimer>1.5){bankTimer=0;const bank=H.bankProgress(run,save);for(const b of bank.bonuses){callout(b.text,2);audio.effect('upgrade');}persist();}updateTutorial();hudTimer+=dt;if(hudTimer>.08){updateHUD();hudTimer=0;}}
